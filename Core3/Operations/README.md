...
A broader operation-pipeline note now also lives in
[OperationPipeline.md](OperationPipeline.md).
...
- boolean is the first explicit partition-and-piece operation
- add and multiply are currently simpler one-result reductions
- traversal/runtime laws are the same family with explicit continuation

So boolean should be treated neither as the model for all operations nor as a
permanent separate subsystem.

Instead it is the first explored case where:

- local correspondences are generated by route partition
- retention may yield zero, one, or many survivors
- carrier inheritance must be explicit

That makes it a useful early prototype for the broader operation pipeline
described in [OperationPipeline.md](OperationPipeline.md).
